...
fugue256_gpu_hash(int thr_id, uint32_t threads, uint32_t startNounce, void *outputHash, uint32_t *resNounce)
{
	...
	*((uint32_t*)mixtabs + (    threadIdx.x)) = tex1D<unsigned int>(mixTab0Tex, static_cast<float>(threadIdx.x));
	*((uint32_t*)mixtabs + (256+threadIdx.x)) = tex1D<unsigned int>(mixTab1Tex, static_cast<float>(threadIdx.x));
	*((uint32_t*)mixtabs + (512+threadIdx.x)) = tex1D<unsigned int>(mixTab2Tex, static_cast<float>(threadIdx.x));
	*((uint32_t*)mixtabs + (768+threadIdx.x)) = tex1D<unsigned int>(mixTab3Tex, static_cast<float>(threadIdx.x));
